...
namespace Utilitarios
{
    public class MyExeption : Exception
    {
        ...
        public MyExeption(string Mensagem, ETipoException Tipo = ETipoException.Erro, string Titulo = "Controle de Vendas",ETamanhoMsgBox Tamanho = ETamanhoMsgBox.Medio)
        {
        ...
        public void OnMyException(object sender, ThreadExceptionEventArgs e)
        {
            ...
            {
                MyMsgBox.Exibir(e.Exception.Message, "Controle de Vendas", EBotoesMyMsgBox.Ok, EIconeMyMsgBox.Aviso, ETamanhoMsgBox.Pequeno);
            }
            else
            {
                MyExeption ex = new MyExeption();
                ex.Detalhes = this.VerificarInnerException(e.Exception);
                ex.Title = e.Exception.TargetSite.IsNull() ? "Controle de Vendas" : e.Exception.TargetSite.Name;

                MyMsgBox.Exibir("Ocorreu um erro inesperado.", "Controle de Vendas", EBotoesMyMsgBox.Ok, EIconeMyMsgBox.Erro, TamanhoMsg, ex);
            }
        }
